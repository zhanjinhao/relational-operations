...
/**
 * @Author ISJINHAO
 * @Date 2021/12/16 19:14
 */
public class StatementGrammarValidator extends StatementVisitorForDelegation<Void> {

    public StatementGrammarValidator(CurdVisitor<Void> client, ROErrorReporter roErrorReporter) {
        super(client);
    ...
    public Void visitComparison(Comparison comparison) {
        ...
        if (leftCurd == null) {
            error(AstROErrorReporterDelegate.STATEMENT_comparison_VALIDATION);
            return null;
        }
        leftCurd.accept(this);
    ...
    private void assertIsNot(Curd comparisonSymbol, Curd right) {
        ...
        if (!(right instanceof Literal)) {
            error(AstROErrorReporterDelegate.STATEMENT_isNot_VALIDATION);
            return;
        }
    ...
    public Void visitBinaryArithmetic(BinaryArithmetic binaryArithmetic) {
        ...
        if (leftCurd == null) {
            error(AstROErrorReporterDelegate.STATEMENT_binaryArithmetic_VALIDATION);
            return null;
        }
        leftCurd.accept(this);
    ...
    public Void visitUnaryArithmetic(UnaryArithmetic unaryArithmetic) {
        ...
        do {
            ...
            if (bangFlag && negativeFlag) {
                error(AstROErrorReporterDelegate.STATEMENT_unaryArithmetic_VALIDATION);
                break;
            }
    ...
    public Void visitGrouping(Grouping grouping) {
        ...
        if (curd == null) {
            error(AstROErrorReporterDelegate.STATEMENT_grouping_VALIDATION);
            return null;
        }
    ...
    public Void visitIdentifier(Identifier identifier) {
        ...
        if (TokenType.IDENTIFIER.equals(name.getType())) {
            ...
            if (!literal.matches("[a-zA-Z_\\d*]+\\.?[a-zA-Z_\\d*]*")) {
                error(AstROErrorReporterDelegate.STATEMENT_identifierName_VALIDATION);
            }
        }
        return null;
    }
    ...
    private void assertFunctionParameter(Curd curd) {
        if (curd instanceof Logic) {
            ...
            error(AstROErrorReporterDelegate.STATEMENT_functionParameter_VALIDATION);
        }
    }
    ...
    public Void visitAssignmentList(AssignmentList assignmentList) {
        ...
        if (entryList.isEmpty()) {
            error(AstROErrorReporterDelegate.STATEMENT_assignmentList_VALIDATION);
            return null;
        }
    ...
    public Void visitTimeInterval(TimeInterval timeInterval) {
        assertTimeType(timeInterval.getTimeType(), AstROErrorReporterDelegate.STATEMENT_timeInterval_VALIDATION);
        return null;
    }

    @Override
    public Void visitTimeUnit(TimeUnit timeUnit) {
        assertTimeType(timeUnit.getTimeType(), AstROErrorReporterDelegate.STATEMENT_timeUnit_VALIDATION);
        return null;
    }
