package cn.addenda.ro.grammar.test.ast.retrieve;
...
public class SelectParserTest {
    ...
    static String[] sqls = new String[]{
        ...
        "select SNO, SNAME, SBIRTHDAY, date_format(now(), 'yyyy') as anow from STUDENT \n" +
            "where date_format(SBIRTHDAY, 'yyyy') = (select date_format(SBIRTHDAY, 'yyyy') from STUDENT where SNO = '108')",
        ...
        "select * from SCORE a where a.CNO = '3-105' " +
            "and a.DEGREE >= (select min(DEGREE) from SCORE b where b.CNO = '3-245') order by DEGREE desc",
